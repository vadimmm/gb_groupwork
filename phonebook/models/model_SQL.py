...
# metadata = db.MetaData()
# engine = db.create_engine('sqlite:///' + DB_SQL_PATH_FULL, echo=True)
#
# connection = engine.connect()
# Session = sessionmaker(bind=engine)
# session = Session()
...
def get_SQL_DeleteContactBy_id():
    ...
    responses = session.query(PhoneBook).filter(PhoneBook.id == field).all()
    if responses == []:
        view.inputStr(f'Контакт с таким ID "{field}" не найден')
    else:
        for result in responses:
            print(result)
            ...
            if delChoice == '1':
                ...
                session.commit()
                view.inputStr(f'Выбранный контакт с таким ID "{field}" удалён!')
                # TODO: ниже должна быть отправка на главное меню
            else:
                view.inputStr(f'Вы отменили удаление контакта с таким ID "{field}"')
                # TODO: ниже должна быть отправка на главное меню
